...
function wilcoxon() {
	...
	if ( !isTypedArrayLike( x ) && !isNumberArray( x ) ) {
		throw new TypeError( format( '1Lx8R', x ) );
	}
	len = x.length;
	if ( arguments.length > 1 ) {
		if ( isObject( arguments[ 1 ] ) ) {
			...
			if ( !isTypedArrayLike( y ) && !isNumberArray( y ) ) {
				throw new TypeError( format( '1LxA5', 'y', y ) );
			}
			if ( len !== y.length ) {
				throw new Error( format('1Lx1E') );
			}
	...
	if ( len < 2 ) {
		throw new Error( format( '1LxA2', x ) );
	}
