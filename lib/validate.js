...
function validate( opts, options ) {
	if ( !isObject( options ) ) {
		return new TypeError( format( '1Lx2V', options ) );
	}
	if ( hasOwnProp( options, 'alpha' ) ) {
		opts.alpha = options.alpha;
		if ( !isNumber( opts.alpha ) || isnan( opts.alpha ) ) {
			return new TypeError( format( '1Lx8P', 'alpha', opts.alpha ) );
		}
		if ( opts.alpha < 0.0 || opts.alpha > 1.0 ) {
			return new RangeError( format( '1Lx8V', 'alpha', opts.alpha ) );
		}
	}
	if ( hasOwnProp( options, 'alternative' ) ) {
		opts.alternative = options.alternative;
		if ( !isString( opts.alternative ) ) {
			return new TypeError( format( '1Lx2W', 'alternative', opts.alternative ) );
		}
		if ( !contains( ALTERNATIVE_VALUES, opts.alternative ) ) {
			return new Error( format( '1Lx4S', 'alternative', ALTERNATIVE_VALUES.join( '", "' ), opts.alternative ) );
		}
	}
	if ( hasOwnProp( options, 'correction' ) ) {
		opts.correction = options.correction;
		if ( !isBoolean( opts.correction ) || isnan( opts.correction ) ) {
			return new TypeError( format( '1Lx2o', 'correction', opts.alpha ) );
		}
	}
	if ( hasOwnProp( options, 'exact' ) ) {
		...
		if (
			...
			return new TypeError( format( '1Lx2o', 'exact', opts.alpha ) );
		}
	}
	if ( hasOwnProp( options, 'mu' ) ) {
		opts.mu = options.mu;
		if ( !isNumber( opts.mu ) || isnan( opts.mu ) ) {
			return new TypeError( format( '1Lx8P', 'mu', opts.mu ) );
		}
	}
	if ( hasOwnProp( options, 'zeroMethod' ) ) {
		opts.zeroMethod = options.zeroMethod;
		if ( !isString( opts.zeroMethod ) ) {
			return new TypeError( format( '1Lx2W', 'zeroMethod', opts.alternative ) );
		}
		if ( !contains( ZERO_METHOD_VALUES, opts.zeroMethod ) ) {
			return new Error( format( '1Lx4S', 'zeroMethod', ZERO_METHOD_VALUES.join( '", "' ), opts.zeroMethod ) );
		}
	}
	return null;
}
